...
export default function ProfileHUD() {
    ...
    return (
        <div className="group relative">
            ...
            <Link href="/profile">
                <div className="flex items-center gap-4 bg-black/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-lg hover:border-cyan-500/50 transition-colors cursor-pointer">
                    <div className="text-right hidden sm:block">
                        <div className="text-white text-sm font-bold group-hover:text-cyan-400 transition-colors">CADET</div>
                        <div className="text-white/40 text-[10px] uppercase tracking-wider">Lvl 1 • CR {credits.toLocaleString()}</div>
                    </div>
                    <div className="w-10 h-10 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-lg ring-2 ring-white/10 group-hover:ring-cyan-500 transition-all shadow-[0_0_15px_rgba(6,182,212,0.3)] flex items-center justify-center">
            ...
            <div className="absolute top-full right-0 mt-4 w-80 bg-[#0f111a]/90 backdrop-blur-xl border border-cyan-500/30 rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50 overflow-hidden">

                <div className="p-4 border-b border-white/10 bg-white/5">
                    <h3 className="text-xs font-mono text-cyan-400 tracking-widest uppercase mb-1">C-Log Profile</h3>
                    <div className="text-lg font-bold text-white">Shivansch</div>
                    <div className="text-xs text-white/40">ID: CD-8842-X</div>
                </div>

                <div className="p-4 space-y-4">
                    <StatRow icon={<Code2 className="w-4 h-4 text-purple-400" />} label="Syntax Mastery" value="12%" color="bg-purple-500" />
                    <StatRow icon={<Cpu className="w-4 h-4 text-blue-400" />} label="Logic Core" value="5%" color="bg-blue-500" />
                    <StatRow icon={<Zap className="w-4 h-4 text-yellow-400" />} label="Efficiency" value="--" color="bg-yellow-500" />
